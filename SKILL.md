...
Root `gabriel.workspace.json` makes the definition validator required and the records
validator conditional on `data/records.json`. Portable schema-v2 repositories normally
omit records. In a Persona workspace, commit and push this child before publishing the
parent lock. The registry contains exactly one shared List.
Unmarked legacy repos retain the parent's loud compatibility fallback, while marked repos
must keep every declared script. A stale parent link is handled by non-destructive `prune`,
which preserves this checkout. Never publish the parent before this child commit is pushed.
